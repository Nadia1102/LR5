...
class csp:
    ...
    def al1_min(self, assignment, csp):
    ...
    def numconflicts(self, A, a, assignment):
        ...
        for item in l:
            if ([k for k, v in self.confederation.items() if str(item) in v][0] == ("Group3")):
                count = count + 1
        ...
        for item in l:
            ...
            elif ([k for k, v in self.confederation.items() if str(item) in v][0] != "Group3"):
                if ([k for k, v in self.confederation.items() if str(A) in v] == [k for k, v in
                                                                                      self.confederation.items() if
                                                                                      str(item) in v]):
    ...
    def recursive_backtracking(self,assignment, csp):
        if len(assignment) == len(self.variables):
            return assignment
        var = self.al1_min(assignment, csp)
